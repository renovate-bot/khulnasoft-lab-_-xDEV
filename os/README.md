...
* If running Bigpatch recovery / install disk from OWC 2.5" SSD, use a 4x PCIe slot (on Mac Pro 3,1 use the 3rd PCIe slot, not the 1st or 2nd 16x slots)
* If Bigpatch app does not run, try resigning it: `codesign --force --deep --sign - /Applications/bigpatch.app`
...
### Installation of macOS 11.2.3 from a Supported Intel Mac 
  ...
  * Open Terminal. These next steps are required to run BigMac2 and CloneToolX from a bigpatch install disk from a Supported Mac:
    * `csrutil`
    * `csrutil disable`
    * `csrutil authenticated-root disable`
   ...
   * Boot from bigpatch
   * From the Springboard, launch bigpatch
   * From bigpatch's pre install tab, Launch the installer. In five seconds, the macOS installer will be shown.
   * Several reboots will happen
   * When welcome screen shows, shutdown.
   * Boot back into the bigpatch intall disk
   ...
   * If success, you can now clone your system to BigMac. Besure to remove the live snapshot on the clone using bigpatch delete snapshot.
...
* This is what we know so far:
  ...
  * For AMD cards MouSSE is not needed. It's only needed on Mojave. I run an RX 580 8GB flashed and a Radeon Pro WX4100 non flashed. Neither need MouSSE. It will be removed from future versions of bigpatch.
