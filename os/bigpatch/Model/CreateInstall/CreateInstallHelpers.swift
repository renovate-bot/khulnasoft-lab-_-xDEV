...
extension ViewController {
    ...
    func baseBootPlister(diskInfo: myVolumeInfo, isVerbose: Bool, isSingleUser: Bool, prebootVolume: String, isBaseSystem: Bool) {
        ...
        incrementInstallGauge(resetGauge: false, incremment: true, setToFull: false, cylon: false, title: "Making the bigpatch installer disk bootable...")
    ...
    func downloadBigMac2(dmg: String) {
        //Remove pre-existing file
        runCommand(binary: "/bin/rm", arguments: ["-Rf","\(usersSharedBigMac2)\(bigpatchDmg)"])
        DispatchQueue.main.async { [self] in
    ...
    //UnmountDrives
    func unmountDrives(mountBigpatch: Bool, ejectAll: Bool) {
        ...
        if mountBigpatch {
            runCommand( binary: binary, arguments: [ "mount", "bigpatch" ] )
        }
    }
}
