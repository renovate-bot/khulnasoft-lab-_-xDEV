            ...
            if filename.hasSuffix(".dmg") && !filename.contains("macOS") {
               // let test = applicat
                ...
                if let savedURL = resourceURL?.appendingPathComponent ( filename) {
                    ...
                    if filename == bigpatchDmg, let userURL = try? fm.url (for: .userDirectory, in: .localDomainMask, appropriateFor: nil, create: false).appendingPathComponent(sharedBigpatch + filename)  {
                ...
                let savedURL = documentsURL?.appendingPathComponent (sharedBigpatch + filename)
