...
class MainFrame(wx.Frame):
    ...
    def _generate_elements(self) -> None:
        ...
        Format:
          - Title label: xDEV Patcher v{X.Y.Z}
        ...
        # Title label: xDEV Patcher v{X.Y.Z}
        title_label = wx.StaticText(self, label=f"xDEV Patcher {'' if self.constants.special_build else ''}{self.constants.patcher_version}{' (Nightly)' if not self.constants.commit_info[0].startswith('refs/tags') else ''}", pos=(-1, 10))
    ...
    def _preflight_checks(self):
        ...
        if "--update_installed" in sys.argv and self.constants.has_checked_updates is False and gui_support.CheckProperties(self.constants).host_can_build():
            ...
            pop_up = wx.MessageDialog(
                self,
                f"xDEV Patcher has been updated to the latest version: {self.constants.patcher_version}\n\nWould you like to update OpenCore and your root volume patches?",
                "Update successful!",
...
Please check the Github page for more information about this release."""
        ...
        self.description = wx.StaticText(panel, label=f"xDEV Patcher {oclp_version} is now available - You have {self.constants.patcher_version}{' (Nightly)' if not self.constants.commit_info[0].startswith('refs/tags') else ''}. Would you like to update?")
