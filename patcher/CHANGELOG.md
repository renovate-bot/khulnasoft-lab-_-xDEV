...
- `OpenCore-GUI.app.zip` removed from release
...
- Add additional error handling for broken settings file from OCLP 2.1.0
  ...
  - Delete `/Users/Shared/.com.dortania.patcher.plist` and restart app to avoid this issue
- Add additional warning about OCLP 2.1.0 bug where certain settings saved incorrectly
  - Delete `/Users/Shared/.com.dortania.patcher.plist` and restart app if `TypeError: unsupported type: <class 'NoneType'>` error occurs
...
- Support for retaining GUI settings when building on-model
  ...
  - Note resetting saved settings not implemented yet
    - Delete `/Users/Shared/.com.dortania.patcher.plist` and restart app to reset settings
...
- Resolve "Label" error in com.dortania.patcher.os-caching.plist.
...
- Restructure project directories
  - Python:
    - Move logic into `patcher` directory
...
- Implement new PKG-based installer
  ...
  - `OpenCore-GUI.app.zip` is deprecated and will be removed in future versions
- Implement new Privileged Helper Tool
  - Removes need for password prompts when installing patches, creating installers, etc.
  - Installed at `/Library/PrivilegedHelperTools/com.dortania.patcher.privileged-helper`
...
- Add new Launch Daemons for handling macOS updates:
  - `macos-update.plist`:
    ...
    - `/Library/LaunchDaemons/com.dortania.patcher.macos-update.plist`
  - `os-caching.plist`
    - Resolves unsupported/old KDKs from being used post-update
    - Invoked when update is downloading
    - `/Library/LaunchDaemons/com.dortania.patcher.os-caching.plist`
...
- Implement basic support for macOS Sonoma:
  - Supports same range of hardware as Ventura, in addition to:
    ...
    - MacBook10,1
    - MacBookPro14,x
      - [T1 chip currently unsupported in Sonoma](https://github.com/khulansoft-lab/patcher/issues/1103)
  - Resolved issues:
...
- Backend changes:
  - Implement new analytics_handler.py module
    ...
    - Can be disabled via GUI or `defaults write com.dortania.patcher DisableCrashAndAnalyticsReporting -bool true`
...
- Add early UHCI/OHCI support (USB1.1)
  ...
  - See associated issue for current limitations: [Legacy UHCI/OHCI support in Ventura](https://github.com/khulansoft-lab/patcher/issues/1021)
...
- Ventura Specific Updates:
  ...
  - Implement downloading from Kernel Debug Kit Backup Repository
    - Alleviates issues with Apple blocking KDK downloads from OCLP (Ref: [Issue #1016](https://github.com/khulansoft-lab/patcher/issues/1016))
...
- Split Kepler userspace and kernel space patches
  ...
  - Reference: [macOS 12.5: Nvidia Kepler and WindowServer crashing #1004](https://github.com/khulansoft-lab/patcher/issues/1004)
...
- Add Nvidia Web Driver support for Maxwell and Pascal
  - Currently running in OpenGL mode, [non-Metal issues](https://github.com/khulansoft-lab/patcher/issues/108) applicable
...
- Add `Patcher.plist` for applied patch info
...
- Implement Automatic Patch Detection/Installation
  ...
  - Notify users when OpenCore is booted from external disk not matching macOS (ie. USB installer)
    - Disable notification via `defaults write com.dortania.patcher AutoPatch_Notify_Mismatched_Disks -bool FALSE`
- GUI Enhancements:
...
- Implement spoofless support (ie. no SMBIOS patching)
  ...
  - See additional notes before updating: [VMM usage notes](https://github.com/khulansoft-lab/patcher/issues/543#issuecomment-953441283)
